\chapter{Background}
One of the greatest problems, but in the same time greatest strength, with deploying applications as a web applications is that they are accessible from everywhere where there exists internet access. This makes the application also easily accessible for people who whishes to harm or abuse the application. Two of the more common security risks are Injection Attack and Cross-Site Scripting. \parencite{OpenWebApplicationSecurityProject, CrossMichael2007Dgtw} These two attacks will be described in this chapter. But to understand the security risks do we first need to understand how a web application work.

The most common architectural design for a web application is based on three tires. The first is the presentation tire which is the visual components often rendered by a browser. The second is the logic tire which can be seen as the brain of the application. The last and third tier is the storage tier, where the second tier can store data as needed. \parencite{JustinClarke-Salt2009SIAa}
