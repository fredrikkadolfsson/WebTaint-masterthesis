\chapter{Result}
\label{Result}
\textit{This chapter presents the results of the conducted evaluation. Appendix \ref{appendix:raw} contains raw data and metrics of data that may not be shown in this chapter. The chapter starts with presenting the results from the \textit{\nameref{Applications}} evaluation where Java-based web applications have been evaluated measuring security vulnerabilities with and without WebTaint. The results from the \textit{\nameref{Performance}} evaluations then follow it where the parameters time and memory is measured.}
